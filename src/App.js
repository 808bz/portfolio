import biliApp from './assets/bili-app_1192x892.png'
import biliAppUi from './assets/bili-app_1283x506.png'
import opsLive from './assets/ops-data_1797x932.png'
import opsMatrix from './assets/ops-data_1266x540.png'

/* ------------------------------------------------------------------ *
 *  Bilingual content.  lang is passed in; L(obj) picks the right side.
 * ------------------------------------------------------------------ */

const CONTENT = {
  brand: { zh: '赵天翔', en: 'Zhao Tianxiang' },
  roleLine: { zh: '内容 · 产品 · AI · 游戏', en: 'Content · Product · AI · Game' },
  statement: {
  ...
  web: { zh: '注：案例页视觉需替换为正式素材', en: 'Note: visual is a placeholder until final assets are ready' },
}
...
const EXPERIENCE = [
  {
    no: '01',
    organization: { zh: '杭州猛犸互动网络科技有限公司', en: 'Hangzhou Mammoth Interactive' },
  ...
  {
    no: '02',
    organization: { zh: '新源县融媒体中心', en: 'Xinyuan Media Center' },
...
export function App(lang, route = '#/') {
  ...
  return `
    ${Nav(lang, isProject)}
    <main>
      ${project ? CaseStudy(project, L) : Home(lang, L)}
    </main>
    ${Footer(L)}
  `
}

function Nav(lang, isProject) {
  const L = (v) => v[lang]
  const sections = [
...
function Work(list, L) {
  return `
  <section id="work" class="work">
    <div class="container">
      <div class="section-head" data-reveal>
        <p class="eyebrow">${list.length} Projects</p>
        <h2 class="h2">${L(CONTENT.selectedWork)}</h2>
      </div>
      ${list.map((p, i) => ProjectItem(p, i, L)).join('')}
    </div>
  </section>
  `
}
  ...
  if (p.visual) {
    ...
    <figure class="figure ${ratio} reveal-img" data-reveal>
      <span class="figure__media figure__media--img">
        <img class="figure__img" loading="lazy" alt="${L(p.title)}" src="${p.visual}" />
      </span>
    </figure>
    `
  }
...
function CaseStudy(p, L) {
  const figs = []
  figs.push(Visual(p, L, 'figure--ratio-169'))
  if (p.secondary) {
    figs.push(`
      <figure class="figure figure--ratio-169 reveal-img" data-reveal>
        <span class="figure__media figure__media--img">
          <img class="figure__img" loading="lazy" alt="${L(p.title)}" src="${p.secondary}" />
        </span>
      </figure>
    `)
  }
  ...
  <section id="experience" class="experience">
    <div class="container">
      <div class="section-head" data-reveal>
        ...
        <h2 class="h2">${L({ zh: '经历', en: 'Experience' })}</h2>
      </div>
      ${EXPERIENCE.map(
        (e) => `
      <div class="exp" data-reveal>
        <div class="exp__head grid">
          ...
          <div class="exp__title">
            <h3 class="exp__org">${L(e.organization)}</h3>
  ...
  <section id="contact" class="contact">
    <div class="container container--text">
      ...
      <div class="contact__links" data-reveal data-reveal-delay="2">
        ...
        <div class="contact__item">
          <p class="contact__label">${L({ zh: '求职意向', en: 'Looking for' })}</p>
